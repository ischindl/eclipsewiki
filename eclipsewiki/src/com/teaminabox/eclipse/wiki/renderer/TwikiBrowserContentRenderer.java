...
public final class TwikiBrowserContentRenderer extends AbstractContentRenderer {
	...
	private static final TextRegionMatcher[]	RENDERER_MATCHERS			= new TextRegionMatcher[] { new IgnoredTextRegionMatcher(), new EmbeddedTextRegionMatcher(new JavaTypeMatcher()), new JavaTypeMatcher(), new EmbeddedTextRegionMatcher(new ForcedLinkMatcher(2)), new ForcedLinkMatcher(2), new WikiSpaceMatcher(), new EscapedWikiWordMatcher(ESCAPED_TWIKI_WORD_PATTERN, '!', TEXT_TO_REMOVE_FOR_ESCAPED), new EmbeddedTextRegionMatcher(new WikiWordMatcher(TWIKI_WORD_PATTERN)),new WikiWordMatcher(TWIKI_WORD_PATTERN), new NonLetterOrDigitMatcher(), new LetterOrDigitMatcher(), new UrlMatcher(), new EmbeddedTextRegionMatcher(new EclipseResourceMatcher()), new EclipseResourceMatcher(), new PluginResourceMatcher() };

	private static final TextRegionMatcher[]	SCANNER_MATCHERS			= new TextRegionMatcher[] { new IgnoredTextRegionMatcher(), new EmbeddedTextRegionMatcher(new JavaTypeMatcher()), new JavaTypeMatcher(), new EmbeddedTextRegionMatcher(new ForcedLinkMatcher(2)), new ForcedLinkMatcher(2), new WikiSpaceMatcher(), new EscapedWikiWordMatcher(ESCAPED_TWIKI_WORD_PATTERN, '!', TEXT_TO_REMOVE_FOR_ESCAPED), new EmbeddedTextRegionMatcher(new WikiWordMatcher(TWIKI_WORD_PATTERN)), new WikiWordMatcher(TWIKI_WORD_PATTERN), new UrlMatcher(), new EmbeddedTextRegionMatcher(new EclipseResourceMatcher()), new EclipseResourceMatcher(), new PluginResourceMatcher() };
